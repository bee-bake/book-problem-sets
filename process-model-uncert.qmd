...
```{r}
#| fig-cap: Figure 9a. Forecast with added total uncertainties for NEE.
combined_df <- bind_rows(parameter_df, process_df, initial_conditions_df, drivers_df)

#for nee
combined_df_nee <- filter(combined_df, variable == "nee") 
sd_df_nee <- combined_df_nee %>%
  group_by(datetime, model_id) %>%
  summarize(sd = sd(prediction, na.rm = TRUE)) %>%
  ungroup()

#Plot the partitioned uncertainties
ggplot() +
    geom_bar(data = sd_df_nee, aes(x = datetime, y = sd, fill = model_id), stat = "identity", position = "stack") +
...
```{r}
#| fig-cap: Figure 9b. Forecast with added total uncertainties for Wood.
#for wood
combined_df_wood <- filter(combined_df, variable == "wood") 
sd_df_wood <- combined_df_wood %>%
  group_by(datetime, model_id) %>%
  summarize(sd = sd(prediction, na.rm = TRUE)) %>%
  ungroup()
...
#Plot the partitioned uncertainties
ggplot() +
    geom_bar(data = sd_df_wood, aes(x = datetime, y = sd, fill = model_id), stat = "identity", position = "stack") +
    ylab("Standard Deviation") +
    xlab("Forecasted date") +
    scale_fill_manual(values = c("process_unc" = "red", "parameter_unc" = "purple", "initial_unc" = "orange",
                                 "driver_unc" = "blue")) +
...
total_unc_df <- output_to_df(new_output, sim_dates, sim_name = "total_unc")

#Plot the graph
total_unc_df |> 
  filter(variable %in% c("nee", "wood")) |> 
  summarise(median = median(prediction, na.rm = TRUE), 
            upper90 = quantile(prediction, 0.95, na.rm = TRUE),
            lower90 = quantile(prediction, 0.05, na.rm = TRUE),
            .by = c("datetime", "variable")) |> 
...
```{r}
#| fig-cap: Figure 10b. Forecast with combined uncertainties for NEE.
#Calculate the variance (SD) and plot 
##For NEE
total_unc_df_nee <- filter(total_unc_df, variable == "nee")
sd_total_df_nee <- total_unc_df_nee %>%
...
#Plot the partitioned uncertainties
ggplot() +
    geom_bar(data = sd_total_df_nee, aes(x = datetime, y = sd, fill = "total_uncertainty"), stat = "identity", position = "stack") +
    ylab("Standard Deviation") +
    xlab("Forecasted date") +
    scale_x_date(date_breaks = "1 day", date_labels = "%b %d") +
    labs(fill = "Uncertainty") +
...
```{r}
#| fig-cap: Figure 10c. Forecast with combined uncertainties for Wood.
##For Wood
total_unc_df_wood <- filter(total_unc_df, variable == "wood")
sd_total_df_wood <- total_unc_df_wood %>%
  group_by(datetime, variable) %>%
  summarize(sd = sd(prediction, na.rm = TRUE)) %>%
  ungroup()
...
#Plot the partitioned uncertainties
ggplot() +
    geom_bar(data = sd_total_df_wood, aes(x = datetime, y = sd, fill = "total_uncertainty"), stat = "identity", position = "stack") +
    ylab("Standard Deviation") +
    xlab("Forecasted date") +
    scale_x_date(date_breaks = "1 day", date_labels = "%b %d") +
    labs(fill = "Uncertainty") +
...
-   If you calculate the variance of the combined uncertainty and compared to the sum of the individual variances, do they match? What does it mean if they are different?
    ...
    Ans: The variance (SD) of combined uncertainty was lower than that evaluated from adding individual uncertainties. This implies that the individual errors/uncertainties/deviations are additive and reinforce each other.
