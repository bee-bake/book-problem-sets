...
```{r}
plot(bart_2019$doy, bart_2019$observation)
```
...
**Answer 3:** No. It doesn't involve the concept of using the most recent previous value to determine the current value.
...
LL_fn <- function(par,x,y) {
  -sum(dnorm(y, mean = pred_logistic(x, par), sd = par[5], log = T))
}
```
...
```{r}
pred_logistic <- function(x, par) {
  y <- fit$par[1] + ((fit$par[2]*exp(fit$par[3]+fit$par[4]*x))/(1+(exp(fit$par[3]+fit$par[4]*x))))  + fit$par[5]
}

bart_2019 <- bart_2019 %>%
  mutate(predicted = pred_logistic(x=bart_2019$doy, par = fit$par))
...
```{r}
res <- bart_2019$predicted - bart_2019$observation
hist(res)
```

**Question 9:** How does the distribution of the data (Question 2) compare to the distribution of the residuals?

**Answer 9:** The distribution of residuals looks a bit inclined towards normal when compared to the distribution of the data.
...
**Answer 12:** The model from 2019 seems to over-predict the values a bit when using it for predicting gcc 2020.
